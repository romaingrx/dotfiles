...
let
  ...
  personalPackages = with pkgs; [ ollama tor mitmproxy ];

  # Work-only packages
  workPackages = with pkgs; [ openbabel zoom-us ];

  # Add work specific packages on linux machine
  linuxWorkPackages = with pkgs; [ onedrive mattermost-desktop ];

  # Determine which role-specific packages to include
  roleSpecificPackages = if config.home.username == "romaingrx" then
    personalPackages
  else
    workPackages;
  ...
  platformSpecificPackages =
    if pkgs.stdenv.isLinux then linuxWorkPackages else [ ];
