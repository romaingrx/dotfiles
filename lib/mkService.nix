...
description ? "${name} service", command, args ? [ ],

# Service behavior
enabled ? true, wantedBy ? [ "default.target" ], after ? [ ], requires ? [ ],

# Process configuration
user ? null, group ? null, workingDirectory ? null, environment ? { },
...
let
  ...
  fullCommand =
    if args == [ ] then command else "${command} ${lib.escapeShellArgs args}";

  # Environment variables as shell format
  envVars = lib.mapAttrsToList
    (name: value: "${name}=${lib.escapeShellArg (toString value)}") environment;
